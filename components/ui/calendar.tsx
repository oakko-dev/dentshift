...
import { th as rth } from "react-day-picker/locale"
...
function Calendar({
	...
	return (
		<DayPicker
			...
			style={{
				width: `${270 * (columnsDisplayed ?? 1)}px`,
			}}
			classNames={{
			...
			formatters={{
				formatCaption: (month, options, dateLib) => {
					const date = dateLib!.format(month, "MMMM yyyy", { locale: th })
					const yearBE = month.getFullYear() + 543 // 2025 + 543 = 2568
					const formattedDate = date.replace(/\d{4}$/, String(yearBE))
					return formattedDate
				},
			}}
			components={{
				Chevron: ({ orientation }) => {
					const iconLoaded = orientation === "left" ? <Icon icon="mdi:chevron-left" className="h-4 w-4" /> : <Icon icon="mdi:chevron-right" className="h-4 w-4" />
					return iconLoaded
				},
				Nav: ({ className }) => (
					<Nav
...
function CaptionLabel({
	...
	return (
		...
		>
			{navView === "days" ? children : `${displayYears.from + 543} - ${displayYears.to + 543}`}
		</Button>
	)
}
